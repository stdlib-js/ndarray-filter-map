...
function filterMap( x, options, fcn, thisArg ) {
	...
	var i;
	if ( !isndarrayLike( x ) ) {
		throw new TypeError( format( '24v5t', x ) );
	}
	...
	if ( !isFunction( clbk ) ) {
		throw new TypeError( format( '24v2b', clbk ) );
	}
	if ( hasOpts ) {
		if ( !isPlainObject( opts ) ) {
			throw new TypeError( format( '24v2V', opts ) );
		}
		...
		if ( hasOwnProp( opts, 'order' ) ) {
			if ( !isOrder( opts.order ) ) {
				throw new TypeError( format( '24v5C', 'order', opts.order ) );
			}
			ord = opts.order;
		}
	} else {
	...
	if ( ctor === null ) {
		...
		throw new TypeError( format( '24vBf', 'dtype', opts.dtype ) );
	}
